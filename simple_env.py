...
class SimpleEnv(AECEnv):
    ...
    def utility(self, action, ag):
        rate = 1
        act = action[ag][0]
        fog = self.world.fogs
        load = self.world.agents[self._index_map[ag]].task.den 
        for agent, _ in action.items():
            if agent is ag:
                continue
            if action[agent] == act:
                rate += 1
                load += self.world.agents[self._index_map[agent]].task.den 
        util = load / fog[act].getDen()
        rate *= 16
        rate += 0.1
        rate = rate**-1
        rate *= 16
        ...
        rate = math.log2(rate)
        rate *= (5 * (10 ** 6))
        util += self.world.agents[self._index_map[ag]].task.ln / rate
        util *= 0.55
        util += self.pay(action, ag)
        return util
    
    def pay(self, action, ag):
        pym = self.world.agents[self._index_map[ag]].task.ln * 3 * 10 ** (-9)
        return pym
    
    def energy(self, action, ag):
        rate = 1
        act = action[ag][0]
        for agent, _ in action.items():
            if agent is ag:
    ...
    def exectime(self, action, ag):
        return (self.utility(action, ag) - self.pay(action, ag)) * 10 / 11
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        agent.action.a = np.zeros(self.world.fog_nb)

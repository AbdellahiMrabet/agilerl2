...
if __name__ == "__main__":
    ...
    path = "./models/MATD3"
    filename = "MATD3_wohpo_trained_agent_fog5_N9.pt"
    ...
    env.close()
    #save csv file
    with open(os.path.join(path, 'score_wohpo_N_9.csv'), 'w', newline='') as file:
    ...
    plt.plot(range(1, len(gscore)+1), gscore, marker='o', label="MATD3_wohpo")
    #plt.title('Individual Agent Rewards over Episodes')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    #plt.grid()
    plt.gca().get_xaxis().set_major_formatter(
        FuncFormatter(lambda x, p: format(int(x * learning_delay), ',')))
    plt.legend()
    plt.savefig(os.path.join(path, 'training_rewards_N_9_wohpo.png'))
    plt.show()
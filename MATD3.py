...
if __name__ == "__main__":
    ...
    def make_env():
        return simple_speaker_listener_v4.parallel_env_19()
    ...
    while np.less([agent.steps[-1] for agent in pop], max_steps).all():
        ...
        gscore.append(np.mean(mean_scores))
        pbar.write(
    ...
    path = "./models/MATD3"
    filename = "MATD3_trained_agent_N_19.pt"
    ...
    env.close()
    #save csv file
    with open(os.path.join(path, 'training_score_N_19.csv'), 'w', newline='') as file:
                writer = csv.writer(file)
                for i in range(len(gscore)):
                    writer.writerow([i, gscore[i]])
    #plot rewards over episodes
    plt.figure()
    
    plt.plot(range(1, len(gscore)+1), gscore, marker='o', label="MATD3")
    #plt.title('Individual Agent Rewards over Episodes')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    #plt.grid()
    plt.gca().get_xaxis().set_major_formatter(
        FuncFormatter(lambda x, p: format(int(x * evo_steps), ',')))
    plt.legend()
    plt.savefig(os.path.join(path, 'training_rewards_N_19.png'))
    plt.show()